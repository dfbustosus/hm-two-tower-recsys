...
def collect_validation_labels(
    ...
    for transaction in transactions:
        if assign_split_bucket(transaction.t_dat, split) != "validation":
            continue
        customer_seen = seen[transaction.customer_id]
    ...
    return {customer_id: tuple(article_ids) for customer_id, article_ids in labels.items()}
