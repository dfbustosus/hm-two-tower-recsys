...
- Expose dependency-light two-tower retrieval smoke validation through `make two-tower-retrieval-smoke CUTOFF=YYYY-MM-DD`; train on cutoff-safe exported examples, retrieve from a bounded exact article pool, and report MAP@12 plus Recall@12/50/100 before promoting it as a candidate source.
