...
- Build article-metadata affinity popularity such as garment-group popularity using only pre-cutoff customer histories and counts.
...
- Expose dependency-light two-tower retrieval smoke validation through `make two-tower-retrieval-smoke CUTOFF=YYYY-MM-DD`; train on cutoff-safe exported examples with pointwise logistic or pairwise BPR loss, retrieve from a bounded exact article pool, and report MAP@12 plus Recall@12/50/100 before promoting it as a candidate source.
- For coverage experiments, support latest-positive-pair and latest-positive-per-customer example selection, but require same-scope MAP/recall and downstream ranker validation before promoting broader but weaker one-positive-per-user models.
- Expose ranker-ready two-tower retrieval rows through `make candidate-export`, `make ranker-baseline`, and `make deterministic-ranker-tuning` with `INCLUDE_TWO_TOWER_RETRIEVAL=1`; tune two-tower-specific deterministic weights only on a previous non-overlapping label window before evaluating them on the requested cutoff.
...
- For bounded content caches, support transaction-popularity-prioritized article subsets with an exclusive priority cutoff no later than the downstream training/evaluation feature cutoff to avoid leakage through partial cache availability.
...
- Evaluate popularity-calibrated content retrieval as a separate source such as `multimodal_similarity_popularity_prior`; promote it only after same-split ranker validation improves MAP@12, not just content-only recall.
